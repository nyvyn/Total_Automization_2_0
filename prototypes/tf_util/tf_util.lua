...
util.empty_sound = function()
  return
  {
    filename = util.path("prototypes/tf_util/empty-sound.ogg"),
    volume = 0
  }
end
...
util.empty_sprite = function()
  return
  {
    filename = util.path("prototypes/tf_util/empty-sprite.png"),
    height = 1,
    width = 1,
    frame_count = 1
  }
...
util.prototype = require("prototypes/tf_util/prototype_util")
...
local collision_layer_aliases = {
  ...
  ["layer-12"] = "car",
  ["layer-13"] = "train",
  ["layer-14"] = "elevated_rail",
  ["layer-15"] = "elevated_train",
  ["layer-16"] = "empty_space",
  ["layer-17"] = "lava_tile",
  ["layer-18"] = "meltable",
  ["layer-19"] = "rail_support",
  ["layer-20"] = "trigger_target",
  ["layer-21"] = "cliff"
}

local function build_collision_mask(entries)
  local mask = {layers = {}}
  for _, entry in pairs(entries) do
    if entry == "not-colliding-with-itself" then
      mask.not_colliding_with_itself = true
    else
      local layer = collision_layer_aliases[entry] or entry
...
util.flying_unit_collision_mask = function()
  return build_collision_mask({"not-colliding-with-itself", "layer-15"})
end

util.ground_unit_collision_mask = function()
  return build_collision_mask({"not-colliding-with-itself", "player-layer", "train-layer"})
end

util.projectile_collision_mask = function()
  return build_collision_mask({"layer-15", "player-layer", "train-layer"})
end
